---
title: volatile关键字解析[转载]
date: 2017-07-16 20:54:00
categories:
- 编程语言
- Java
tags:
...
![这里写图片描述](https://objects.yongtao.wang/images/20170716/212219343783699.jpg)
...
![这里写图片描述](https://objects.yongtao.wang/images/20170716/212305263939989.jpg)
...
i  = 10;
```
 　　执行线程必须先在自己的工作内存中对变量i所在的缓存行进行赋值操作，然后再写入主存当中。而不是直接将数值10写入主存当中。
...
　　另外，通过synchronized和Lock也能够保证可见性(修饰共享变量的setter or getter)，synchronized和Lock能保证同一时刻只有一个线程获取锁然后执行同步代码，并且在释放锁之前会将对变量的修改刷新到主存当中。因此可以保证可见性。
#### 有序性
